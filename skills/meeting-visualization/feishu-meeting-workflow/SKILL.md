...
description: Use when Codex needs to analyze Feishu/Lark meeting notes, AI notes, Meeting transcript docs, local meeting transcripts, or meeting Markdown files; guide users through workspace selection, optional CRM skill setup, Lark CLI installation/configuration/profile selection, resolve AI notes to the original transcript, create a local meeting case, perform AI-based meeting analysis from the transcript, optionally route presales cases to an explicitly provided CRM skill, and render Markdown analysis into HTML.
---
...
- Do not assume the CRM skill location. For presales routing, use a user-provided local CRM skill path or a user-provided GitHub repository URL.
...
3. **Is CRM needed?** If the meeting is presales or the user asks for CRM, ask for either a local CRM skill directory containing `SKILL.md` or a GitHub HTTPS repo URL. If unknown, continue case setup with `crm_skill_path: ""` and stop before invoking CRM.
...
Load `references/guided_setup.md` when the user is installing/configuring Lark CLI, deciding where to put CRM, or stuck at a permission/profile step.
...
If the user says to use the customer presales/CRM skill, use `--meeting-type presales` and `--crm-stage 纪要|成果|问卷|会前|提问` as appropriate. The CRM skill location must be provided explicitly by path, environment variable, or GitHub URL.

CRM dependency options:

- Local CRM skill: pass `--crm-skill-path "<path-containing-SKILL.md>"`.
- GitHub CRM skill: pass `--crm-skill-git-url "https://github.com/<owner>/<repo>"`; add `--crm-skill-subdir "<subdir>"` if `SKILL.md` is not at the repo root.
- If neither is provided, create the case and `crm_handoff.md`, but leave `crm_skill_path` empty and ask the user for the CRM skill location before invoking CRM.
...
- `presales`: use the CRM skill only when the user provides `--crm-skill-path`, `FEISHU_MEETING_CRM_SKILL_PATH`, or `--crm-skill-git-url`. Keep CRM input restricted to customer-visible material. Do not guess the CRM skill path from the author's machine or from common home-directory layouts.
...
The current renderer is a simple standalone HTML renderer. For high-quality customer-visible pages, build a custom HTML page rather than relying only on this generic renderer.
