...
- **社区登录不等于创作平台登录**：发布任务必须确认 `https://creator.xiaohongshu.com/publish/publish?source=official` 可进入发布页；如果跳到 `/login`、出现 `401`、短信登录页或"发送验证码"，先让用户完成创作服务平台登录，再上传内容。
- **只填预览时绝不发布**：用户要求"预览 / 填好 / 不发布"时，只运行 `fill-publish` 或 `fill-publish-video`；不得运行 `click-publish`。最终发布必须由用户明确确认或手动完成。
- **用户明确给 tags 时保持精确文本**：不要用相近推荐话题替换用户 tag。只有联想项文本与 tag 精确匹配时才点击；否则保留正文末尾原始 `#tag` 文本。
...
- 优先使用网页端更稳定的视频格式：H.264 MP4、1080p 或接近 1080p、`faststart`。
- 如果原始 `.mov`、超高分辨率录屏或大文件上传后页面显示"上传失败 / 网络异常"，先不要重填正文；将视频转为轻量 MP4 后重新运行 `fill-publish-video`。
- 建议转码命令：

```bash
ffmpeg -y -i "/abs/path/source.mov" \
  -vf "scale=-2:1080" \
  -c:v libx264 -preset veryfast -crf 23 \
  -movflags +faststart -an \
  /tmp/xhs_publish_preview.mp4
```

- 新版创作页可能没有旧版 `.publish-page-publish-btn button.bg-red` selector。判断视频是否上传成功时，优先看页面是否显示文件名、没有"上传失败/网络异常"、出现视频预览和"笔记预览/封面预览"；不要把旧按钮 selector 缺失直接当作上传失败。
...
- **创作平台未登录**：如果发布页进入短信登录页、`401` 或显示"发送验证码"，说明 creator 登录态缺失；让用户完成创作服务平台登录后再重试。
- **图片下载失败**：提示更换图片 URL 或改用本地图片。
- **视频上传失败/网络异常**：优先转码为 1080p H.264 MP4 后重试；不要把已失败的视频页面继续当作可发布预览。
- **视频处理超时**：视频上传后需等待处理（最长 10 分钟）。超时后先检查页面是否已有文件名、视频预览和笔记预览；若已有，可继续填表；若显示失败，转码重试。
- **标题过长**：自动缩短标题，保持语义。
