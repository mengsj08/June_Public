...
def cmd_topic2feishu(args: argparse.Namespace) -> None:
    """Coze topic2feishu_xhs workflow: collect notes, merge analysis, write Feishu."""
    ...
    if args.input_json:
        state = load_workflow_json(args.input_json)
        result = WorkflowResult(
            keyword=state.get("keyword") or args.keyword or "",
        ...
        if not args.keyword:
            _output(
                {
                    "success": False,
                    "error": "--keyword is required unless --input-json is set",
                },
                exit_code=2,
            )
            return
        ...
        try:
            notes, failures = collect_notes(
                ...
                location=args.location or "",
                load_all_comments=args.load_all_comments,
                detail_wait_min=args.detail_wait_min,
                detail_wait_max=args.detail_wait_max,
            )
            result = WorkflowResult(keyword=args.keyword, notes=notes, failures=failures)
        finally:
            browser.close()
    ...
    records, record_failures = build_records(
        result.notes,
        analysis,
        require_analysis=args.write_feishu and not args.allow_empty_analysis,
    )
    ...
    if args.write_feishu:
        if not args.base_token or not args.table_id:
            _output(
                {
                    "success": False,
                    "error": "--base-token and --table-id are required with --write-feishu",
                },
                exit_code=2,
            )
            return
        ...
        result.feishu = write_feishu_records(
            records=result.records,
            base_token=args.base_token,
            table_id=args.table_id,
            lark_as=args.lark_as,
            lark_profile=args.lark_profile or "",
            skip_field_schema_check=args.skip_field_schema_check,
    ...
    data["success"] = True
    if args.output_json:
        save_workflow_json(args.output_json, data)
        data["output_json"] = args.output_json
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub = subparsers.add_parser(
        "topic2feishu",
        help="关键词采集小红书笔记、合并文案分析并写入飞书 Base",
    )
    sub.add_argument("--keyword", help="小红书搜索关键词；使用 --input-json 时可省略")
    sub.add_argument("--number", type=int, default=10, help="处理笔记数量 (default: 10)")
    sub.add_argument("--sort-by", default="综合", help="排序: 综合|最新|最多点赞|最多评论|最多收藏")
    sub.add_argument("--note-type", default="图文", help="类型: 不限|视频|图文")
    sub.add_argument("--publish-time", help="时间: 不限|一天内|一周内|半年内")
    sub.add_argument("--search-scope", help="范围: 不限|已看过|未看过|已关注")
    sub.add_argument("--location", help="位置: 不限|同城|附近")
    sub.add_argument("--load-all-comments", action="store_true", help="详情阶段加载全部评论")
    sub.add_argument("--detail-wait-min", type=float, default=10.0, help="每 3 篇后的最小等待秒数")
    sub.add_argument("--detail-wait-max", type=float, default=20.0, help="每 3 篇后的最大等待秒数")
    sub.add_argument("--input-json", help="读取既有采集结果，避免重复访问小红书")
    sub.add_argument("--output-json", help="保存完整 workflow 结果到 JSON 文件")
    sub.add_argument("--analysis-json", help="文案分析 JSON 字符串、文件路径，或 @file")
    sub.add_argument("--write-feishu", action="store_true", help="写入飞书 Base")
    sub.add_argument("--base-token", help="飞书 Base app/base token")
    sub.add_argument("--table-id", help="飞书 Base table id 或表名")
    sub.add_argument("--lark-as", default="user", choices=["user", "bot"], help="lark-cli 身份")
    sub.add_argument("--lark-profile", help="lark-cli profile 名称")
    sub.add_argument("--skip-field-schema-check", action="store_true", help="跳过飞书字段结构读取")
    sub.add_argument("--allow-empty-analysis", action="store_true", help="允许分析字段为空时写入")
    sub.add_argument("--dry-run", action="store_true", help="传给 lark-cli 的 dry-run")
    sub.set_defaults(func=cmd_topic2feishu)
