...
description: |
  小红书自动化技能集合。支持认证登录、内容发布、搜索发现、社交互动、复合运营、关键词采集写入飞书。
  当用户要求操作小红书（发布、搜索、评论、登录、分析、点赞、收藏、采集入库、写入飞书 Base）时触发。
version: 1.0.0
metadata:
  openclaw:
    requires:
...
5. **采集改写入库**（"topic2feishu / 小红书关键词采集写入飞书 / 文案分析改写入库"）→ 执行 `topic2feishu-xhs` 技能。
...
### topic2feishu-xhs — 采集改写入库

按关键词搜索小红书笔记，获取详情，由 agent 生成营销文案分析和改写，再批量写入飞书 Base。

## Onboarding

当用户第一次使用或迁移给新用户时，先完成账号连接自检：

1. 确认 Chrome 已加载本项目 `extension/` 目录下的 XHS Bridge 扩展。
2. 让用户在 Chrome 中登录自己的小红书账号。
3. 运行 `python scripts/cli.py check-login`；成功后才能继续小红书操作。
4. 如果要写入飞书，确认本机存在可用的 `lark-cli` profile，并让用户提供 `base_token`、`table_id`、`lark_profile` 和 `lark_as`。
5. 用 `lark-cli --profile <profile> base +field-list --base-token <base_token> --table-id <table_id> --as <user|bot>` 验证目标表权限。
...
- **Chrome 或扩展未连接**：提示用户打开 Chrome、加载 `extension/`，再运行 `check-login`。
- **操作超时**：检查网络连接，适当增加等待时间。
- **频率限制**：降低操作频率，增大间隔。
- **飞书权限失败**：优先检查 `lark-cli` profile、组织、授权 scope、`--as user|bot` 和目标 Base 访问权限。
