...
function connect() {
  ...
  ws.onerror = (e) => {
    // The local bridge is often started on demand by the CLI, so connection
    // failures during Chrome startup are expected and should not mark the
    // extension as broken in chrome://extensions.
    console.warn("[XHS Bridge] WS 连接暂不可用", e);
  };
}
