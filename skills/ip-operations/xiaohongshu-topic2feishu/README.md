...
| **topic2feishu-xhs** | 采集入库 | 关键词采集、笔记详情、文案分析改写、写入飞书 Base |
...
本项目可以整体迁移给其他用户使用，但迁移的是代码、skill 和浏览器扩展，不迁移任何账号授权。
...
- `.venv/`
- `.topic2feishu-runtime/`
- `.lark-cli/`
- `.pytest_cache/`
- Chrome profile、cookies、登录态
- 飞书 app secret、user token、bot token
- 本地采集输出 JSON
...
返回昵称或账号信息即表示 XHS Bridge、浏览器和登录态已连通。若未登录，先在 Chrome 里完成扫码或验证码登录，再重新检查。

### 第四步：连接自己的飞书账号

`topic2feishu-xhs` 写入飞书 Base 时使用本机 `lark-cli`。每个使用者都应配置自己的飞书应用、组织和授权 profile，不要复用他人的本地配置。

添加 profile：

```bash
lark-cli profile add --name <profile-name> --app-id <app-id> --brand feishu --app-secret-stdin
```

完成用户授权：

```bash
lark-cli --profile <profile-name> auth login --scope "base:field:read base:record:create"
```
...
- `91403 you don't have permission` 通常是 profile/组织不对、用户未授权、应用 scope 不足，或目标 Base 没有给当前用户/机器人授权。
- 使用 `--as user` 时，授权用户本身必须能访问目标 Base。
- 使用 `--as bot` 时，机器人必须被加入目标 Base，且应用有写入权限。
...
**采集并写入飞书：**
> "搜索关键词 AI for science，抓取 5 条信息，分析改写后写入这个飞书 Base"
...
| `topic2feishu` | 关键词采集小红书笔记、合并文案分析并写入飞书 Base |
| `publish` | 一步发布图文 |
...
│   │   ├── topic2feishu.py         # 小红书关键词采集 + 飞书 Base 写入
...
lark-cli --profile <profile-name> base +field-list --base-token <base-token> --table-id <table-id> --as user
PYTHONPATH=scripts python -m pytest tests/test_topic2feishu.py
```
