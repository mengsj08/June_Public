...
description: 在 macOS 本地启动科研长 PDF 双语阅读工作台，上传文本型或扫描版英文 PDF；可靠文本页直接提取，扫描页按需用 PaddleOCR 生成可搜索文本层，再通过用户已有的 Codex 或 Claude 订阅翻译为中文 PDF，并提供同页左右对照、任务留存和文件导出。适用于科研论文、技术报告、百页级英文 PDF 的中文阅读；不适用于多语言扫描件、复杂公式重排或要求出版级版式复刻的任务。
---
...
16. 用户对告警的“忽略/恢复待复核”、诊断和候选版本状态写入 `review-state.json` 与 `repairs/`，不得改写原始 `qa-alpha.json`。若失败，先读取页面显示的可核对错误，再看任务目录中的 `task.json`；不要把原文、日志或凭据复制到聊天或提交中。

## 硬边界

- 仅监听 `127.0.0.1`，不创建公网服务。
- UI 提供“退出程序”；仍有排队或运行任务时必须拒绝退出，避免损坏翻译任务。
...
Windows 暂不支持：本版本只在 macOS（Apple Silicon）上完成验证与验收；Windows 适配属实验方向，未随本包发布。
