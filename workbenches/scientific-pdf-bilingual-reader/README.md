...
服务与任务文件保存在本机并只监听 `127.0.0.1`，不需要 API Key；选择 Codex 或
Claude 翻译、审阅时，完成任务所需的文字或页面截图会发送给相应的模型服务。

当前公开快照已包含 Comment、逐条 AgentReview、整篇审校进度、跨页批量送审、批准
修复池、逐页裁定与最终候选的原子安装。跨 PDF 经验候选库仍未实现。

## 交给本地 Agent 安装

如果你使用 CodeDesk、CodeBar、Codex、Claude 或其他本地 coding agent，可以直接把
本目录交给它，并要求它先完整阅读 [`AGENT_SETUP.md`](AGENT_SETUP.md)。Agent 会负责
环境检查、受管运行时安装、启动与验活；大型下载、替换已有 Skill 或安装 OCR 前必须先
征得你的确认。

可直接发送：

> 阅读 `AGENT_SETUP.md`，替我检查、安装并启动这个工作台。安全检查由你执行；大型
> 下载、覆盖已有安装或安装 OCR 前先问我，不要读取凭据或暴露我的 PDF。
...
质量复核记录、Comment、AgentReview、人工裁定和修复候选保存在任务目录中；候选被
接受前不会覆盖当前可下载 PDF，接受失败会回滚整组输出。

## 隐私与边界

- 服务只绑定 `127.0.0.1`；任务数据在 `~/.local/share/scientific-pdf-bilingual-reader/`。
- 安装器不读取、不导出任何凭据；翻译走本机 CLI 登录态。
- Codex 文本与审阅运行在空白临时工作区并禁用执行、应用、浏览器和多 Agent 工具；
  Claude 文本翻译禁用工具，截图审阅只开放临时目录内两张复制图片的读取权限。
- 使用模型时会发送必要的文字或页面截图；不要把本工作台理解为离线模型。
